...
class Rod:
    def __init__(self, node_dof, node_num, node_coord, uni_num, node_unit, force, constraints, materials, uni_ma, uni_type='rod'):
        ...
        uni_num 单元个数
        node_num 节点个数
        node_unit 记录单元与节点的对应关系 2*uni_num
        ...
        for i in range(uni_num):
            units.append(Unit(i+1, materials[uni_ma[i]-1], node_dof, nodes[node_unit[0,i]-1], nodes[node_unit[1,i]-1], uni_type))
    ...
    def AK_g(self): #组集合成结构刚度阵
        ...
        #for m in range(self.uni_num):
            ...
            for i in range(self.node_dof*2):
    ...
    def Output(self):
        ...
        #for i in range(self.uni_num):
            ...
            T = uni.TransformMatrix()  #坐标变换
            TT = uni.TransformMatrixT() #坐标变换的转置
            TK = uni.Stiffness_u() #单元刚度阵（局部坐标）
            s_change = self.s_change(uni.id-1)
            ...
            for j in range(2):
                for k in range(self.node_dof*2):
                    dee[int(uni.id-1),j] += T[j,k]*deg[int(uni.id-1),k]
                    
            for j in range(2):
                for k in range(2):
                    fee[int(uni.id-1),j] += TK[j,k]*dee[int(uni.id-1),k]
            
            A = self.units[int(uni.id-1)].material.A
            #print(A)
            sigma[int(uni.id-1)] = fee[int(uni.id-1),1]/A
            
            for j in range(self.node_dof*2):
                for k in range(2):
                    f[int(uni.id-1),j] += TT[j,k]*fee[int(uni.id-1),k]
        ...
        #for i in range(self.uni_num):
            ...
            for j in range(2*self.node_dof):  
